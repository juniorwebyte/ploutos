...
<head>
  <meta charset="UTF-8">
  <title>Cupom Fiscal - Cancelamentos</title>
  <style>
    @page {
      size: 80mm auto;
      margin: 0;
    ...
    * {
    ...
    /* Negrito em todo o cupom de cancelamentos */
    body, .container, .header, .section, .section-title, .cancelamento-item, .cancelamento-header, .row, .total, .footer, h1, span, div {
      font-weight: bold;
    }
    html, body {
      width: 100%;
...
<body>
  ...
  <div class="header">
    ...
    <h1>CUPOM FISCAL DE CANCELAMENTOS</h1>
    <div>${BUSINESS_CONFIG.COMPANY.NAME}</div>
    <div>${BUSINESS_CONFIG.COMPANY.ADDRESS}</div>
    <div>${BUSINESS_CONFIG.COMPANY.CITY}</div>
  ...
  ${cancelamentos.length === 0 ? `
    <div class="section" style="text-align: center; padding: 20px;">
      <div style="font-weight: bold;">NENHUM CANCELAMENTO REGISTRADO</div>
      <div style="font-size: 10px; margin-top: 5px;">Não há cancelamentos para o dia de hoje</div>
    </div>
  ` : `
    <div class="section">
      ${cancelamentos.map((cancelamento, index) => `
        <div class="cancelamento-item">
          ...
          ${cancelamento.descricaoMotivo && cancelamento.descricaoMotivo.trim() !== '' ? `
          <div class="descricao">
            <div class="descricao-label">Descrição:</div>
            <div class="descricao-texto">${String(cancelamento.descricaoMotivo).replace(/\n/g, '<br>')}</div>
          </div>
          ` : ''}
